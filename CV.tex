...
\begin{tikzpicture}[remember picture, overlay] 
     \node[anchor = north east] at ($(current page.north east)+(-2cm,-1.2cm)$) {\includegraphics[height=2.5cm]{cv_photo2.png}};
  \end{tikzpicture}%
  %%%% 利用tikz来定位学校Logo，这里只在第一页显示
  \begin{tikzpicture}[remember picture, overlay] 
    \node[anchor = north west] at ($(current page.north west)+(0.5cm,+1.0cm)$) {\includegraphics[height=6cm]{zju.png}};
  \end{tikzpicture}%
\centerline{\LARGE\bfseries{叶储睿}} 
...
数学科学学院\quad 大二\quad GPA 3.87/4.00
\begin{itemize}[nosep]
...
项目描述：针对工业智能化中高维复杂模型假设检验的难题，项目旨在突破统计功效失真与计算效率低下的权衡瓶颈，提出了一种集高精度、高效率与强稳定性于一体的通用推断框架，相关成果以第一作者身份发表于 \textbf{《Expert Systems with Applications》}。
...
\begin{itemize}[nosep, topsep=\intraProjectListTopSep]
    ...
    \item \textbf{工程与性能优化}：成功减小了小样本与非高斯噪声条件下的假阳性率；并结合拟蒙特卡洛的高维均匀性，大幅缩减了尾部概率估算所需的采样量与计算耗时。
    \item \textbf{核心产出与应用}：验证了框架在半导体缺陷检测及金融风险监控中的实用性，实现了高维统计推断、方差缩减、平滑函数建模到工业级实时响应的验证。
\end{itemize}
...
项目描述：基于钱学森《星际航行概论》拼接圆锥曲线理论，扩展并构建了结合月球引力弹弓效应与 JPL Horizons 真实历表数据的工程级行星际轨道优化与四体数值动力学仿真系统。
\begin{itemize}[nosep, topsep=\intraProjectListTopSep]
    \item \textbf{动力学建模与数值积分}：基于 \textbf{Python/SciPy} 构建了四体受力模型，采用 Velocity-Verlet 辛积分器解决了高维非线性轨道演化的能量漂移问题。
    \item \textbf{发射窗口优化、工程对接与可重现性}：设计内外层双重网格扫描与优化算法，在 2026 年 365 天窗口内成功搜索出最小总速度增量的最优发射日期与轨道参数；接入 \textbf{JPL Horizons API} 获取真实天体状态向量，完成数值轨道的真实历表残差校验；编写 \textbf{Makefile} 实现数据抓取、仿真计算、分析绘图及轨道动画渲染。
\end{itemize}

\section{\makebox[\widthof{\faCogs}][c]{\color{CVBlue}\faCogs}\ 技术栈}
    \textbf{编程语言：} Python, R \quad \textbf{开发工具：} LaTex, Git等
\section{\makebox[\widthof{\faGraduationCap}][c]{\color{CVBlue}\faList}\ 获奖情况}
\begin{itemize}
    \item 浙江大学2024-2025学年二等奖学金 \hfill 2025.10
    \item 浙江大学2024-2025学年学业优秀标兵 \hfill 2025.10
    \item 2026年美国大学生数学建模竞赛Honorable Mention奖 \hfill 2026.02
\end{itemize}
    
\section{\makebox[\widthof{\faInfo}][c]{\color{CVBlue}\faInfo}\ 其他}
\begin{itemize}[parsep=0.5ex]
    \item \textbf{英语水平：} TOFEL 110, CET-4 605, CET-6 617
