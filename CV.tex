...
\begin{itemize}[nosep, topsep=\intraProjectListTopSep]
    ...
    \item \textbf{发射窗口优化、工程对接与可重现性}：设计内外层双重网格扫描与优化算法，在 2026 年 365 天窗口内成功搜索出最小总速度增量的最优发射日期与轨道参数；接入 \textbf{JPL Horizons API} 获取真实天体状态向量，完成数值轨道的真实历表残差校验；编写 \textbf{Makefile} 实现了涵盖数据抓取、仿真计算、分析绘图到轨道动画渲染的全自动流水线。
\end{itemize}
